/* Program to seperate k vertices in a weighted graph such that no path exist b/w any two vertices belonging to the mentioned k nodes. This should be done 
 * in way which requres minimum weight to destroy the paths. This program takes input a graph and k nodes and output is the the minimum total minimum weight 
 * required of the destroyed edges.*/
...
int main() {
	...
	cout<<"ENTER THE NO OF VERTICES, NO OF ROBOTS AND NO OF EDGES";
	cin>>n>>k>>no_of_edges;
	...
	cout<<"ENTER THE EDGE VERTICES AND ITS WEIGHT (u,v,w where u,v is an edge and w is the weight) : ";
	for(int i=0;i<no_of_edges;i++) {
	...
	int RobotPosition[k];
	cout<<"ENTER THE K VERTICES : ";
	for(int i=0;i<k;i++)
		cin>>RobotPosition[i];
	mergeSort(RobotPosition,0,k-1);
	DIVIDE(n,EDGES,RobotPosition,k);
	cout<<"THE MINIMUM WEIGHT OF THE DESTROYED EDGES IS : "<<minTime;
	return 0;
}
